locals {
  naming = { for i in var.environments : i => {
    google_project = {
      ...
      # project_id = lower(replace(join("-", [var.name, i, random_string.suffix[0].id]), "/[\\s_]/", "-")) # Replace all upper letters to lower, inner spaces and underscores to dashes.
      project_id = lower(replace(join("-", [var.name, i]), "/[\\s_]/", "-"))
